    ...
    test("1. 全コードポイント(0x0000-0xFFFF) 穴埋め一貫性テスト", () => {
        ...
        for (let i = 0x0000; i <= 0xFFFF; i++) {
            ...
            if (i >= 0x0040 && i <= 0x007E) {
                // 64-126: 半角カタカナ (U+FF61 - U+FF9F) 計63文字
                const expectedHalfKana = String.fromCodePoint(0xFF61 + (i - 0x0040));
                expect(utf8).toBe(expectedHalfKana);
            }
            
            if (i >= 0x007F && i <= 0x0097) {
                // 127-151: ギリシャ文字小文字 (U+03B1 - U+03C9) 計25文字
                const expectedGreek = String.fromCodePoint(0x03B1 + (i - 0x007F));
            ...
            if (i === 0x0143) expect(utf8).toBe("ぁ"); // U+3041 (ひらがな開始: 0x103 + 64文字)
            if (i === 0x01A2) expect(utf8).toBe("ァ"); // U+30A1 (カタカナ開始: 0x143 + 95文字)
            if (i === 0x0201) expect(utf8).toBe("！"); // U+FF01 (全角英数開始: 0x1A2 + 95文字)
            if (i === 0x025F) expect(utf8).toBe("一"); // 漢字開始位置 (0x201 + 94文字)
        }
